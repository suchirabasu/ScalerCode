...
int Solution::seats(string A) {
    ...
    if(positions.size() <= 1) return 0; //Edge case

    int mid_idx = (positions.size()-1) >> 1;
   
    for(int itr = mid_idx-1; itr >= 0; itr--){
        if(positions[itr+1] - positions[itr] > 1 ){
            ans += positions[itr+1] - positions[itr]-1;
            ans %= 10000003;
            positions[itr] = positions[itr+1] - 1;
        }
    }
    for(int itr = mid_idx+1; itr < positions.size(); itr++){
        if(positions[itr] - positions[itr-1] > 1 ){
            ans += positions[itr] - positions[itr-1]-1;
            ans %= 10000003;
            positions[itr] = positions[itr-1] + 1;
        }
    }
