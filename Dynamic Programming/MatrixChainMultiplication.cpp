/*
Q32. Matrix Chain Multiplication
Problem Description
...
int Solution::solve(vector<int> &A) {
    vector<vector<int>> minCost(A.size(), vector<int>(A.size(), INT_MAX));
    for(int i=A.size()-1;i>=1;i--){
        for(int j=i;j<A.size();j++){
            ...
            for(int k=i;k<j;k++){
                minCost[i][j] = min( minCost[i][j], minCost[i][k] + minCost[k+1][j] + ( A[i-1] * A[k] * A[j] ) );
            }

        }
    }
    return minCost[1][A.size()-1];
}