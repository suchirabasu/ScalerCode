...
int Solution::calculateMinimumHP(vector<vector<int> > &A) {
    ...
    vector<int> pre(m,0);
    vector<int> curr(m,0);
    pre[m-1] = A[n-1][m-1]>=0? 1:abs(A[n-1][m-1])+1;
   
    for(int j=m-2; j>=0; j--){
        pre[j] = max(1, pre[j+1] - A[n-1][j]);
    }
    int pre_val = pre[m-1];
    for(int i=n-2; i>=0; i--){
        curr[m-1] = max(1, pre_val - A[i][m-1]);
        pre_val = curr[m-1];
        for(int j=m-2; j>=0; j--){
            curr[j] = max(1, min(pre[j], curr[j+1]) - A[i][j]);
        }
        pre = curr;
    }
    return pre[0];
