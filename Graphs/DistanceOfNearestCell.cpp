...
vector<vector<int> > Solution::solve(vector<vector<int> > &A) {
    ...
    while(!q.empty()){
        int sz=q.size();
        while(sz--){
            pair<int,int>vec=q.front();
            q.pop();
            //updating nearest distance to its djacent top,bottom,up,down direction
            for(int k=0;k<4;k++){
                int x=vec.first+dir[k].first;
                int y=vec.second+dir[k].second;
                if(x<n && x>=0 && y<m && y>=0){
                    if(ans[x][y] > ans[vec.first][vec.second]) {
                    ans[x][y] = ans[vec.first][vec.second]+ 1;
                    q.push({x,y});
                    }
                }
            }
